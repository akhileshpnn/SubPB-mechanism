...
class BasinBoundary:
    ...
    def find_boundary(self):
        ...
        """
        threshold is a free parameter in the method that determines the Ubound.
        Thereshold curvature value is used to find high curvature areas that maybe
        a steady state. For a new model one needs to optimize this thereshold value.
        """
        ...
        for idx in range(len(xc)):
            coord=tuple([int(np.ceil(xc[idx])),int(np.ceil(yc[idx]))])
            # coord=tuple([int(xc[idx]),int(yc[idx])])
            a=self.Xpot[coord]
            b=self.Ypot[coord]    
               
            Zy_b=Zy[np.where(labels==idx+1)]
            Zx_b=Zx[np.where(labels==idx+1)]
            ...
            if (np.min(Zx_b)<0 and  np.min(Zy_b)<0) and (np.max(Zx_b)>0 and  np.max(Zy_b)>0):
                ...
                X_bndry=self.Xpot[np.where(bndry==1)]
                Y_bndry=self.Ypot[np.where(bndry==1)]
